...
$server = Start-Process -FilePath 'python' -ArgumentList @('-m', 'http.server', '8751', '--bind', '127.0.0.1') -WorkingDirectory $root -PassThru -WindowStyle Hidden
Start-Sleep -Milliseconds 1200
try {
  $edge = 'C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe'
  if (-not (Test-Path $edge)) { $edge = 'C:\Program Files\Microsoft\Edge\Application\msedge.exe' }
  $shots = @(
    @{ url = 'http://127.0.0.1:8751/projects/'; out = 't-projects.png'; w = 1440; h = 2200 },
    @{ url = 'http://127.0.0.1:8751/nl/projects/'; out = 't-nl-projects-mobile.png'; w = 500; h = 4200 }
  )
  foreach ($s in $shots) {
